...
public class UDPManager{
    ...
    public ConcurrentHashMap<Integer, ConcurrentLinkedQueue<RTPPacket>> content = new ConcurrentHashMap<Integer, ConcurrentLinkedQueue<RTPPacket>>();
    ...
    public void startStream(int StreamId, String pathToFile, TCPManager tcpManager){
        try{
            ...
            new Thread(() -> {
                ...
                for (int i = 1; i < packetManager.getNumberOfFrames(); i++){
                    ...
                    try {
                        ...
                        System.out.println("Sending packet: "+ rtpPacket.getsequencenumber());
                        System.out.println("Packet size: "+ rtpPacket.payload_size);
                        System.out.println("Payload len:"+ rtpPacket.getpayload_length());
                        socket.send(packet);
                    } catch (Exception e) {
                ...
                //Rever isto
                tcpManager.sendPacket(null, new TCPPacket(TCPPacket.Type.STREAM_END));
    ...
    private void rpRedirect(DatagramPacket pac){
        ...
        System.out.println("Requesting streams nodes: "+ this.streamSockets.toString());
    ...
    public void handleResponse(DatagramPacket packet){
        try{
            ...
            if(this.node.getNodeType()==Node.type.client){
                System.out.println("Received packet from: "+packet.getAddress().toString());
                int streamId = packet.getData()[0];
                System.out.println("StreamId: "+streamId);
                if(!this.content.containsKey(streamId)){
                    this.content.put(streamId, new ConcurrentLinkedQueue<RTPPacket>());
                }
                RTPPacket pac = new RTPPacket(packet.getData(), packet.getLength());
                this.content.get(streamId).add(pac);
                System.out.println("Packet with stream Id added"+ pac.getsequencenumber());
                System.out.println("Number of streams: "+ this.content.size());
            }
